...
export function createServer({ clientFactory, updateChecker = checkForUpdate } = {}) {
  ...
  const server = new McpServer({ name: 'palatial', version: VERSION }, {
    instructions: 'Use Palatial when the user requests simulation-ready 3D assets from text, images, or CAD. Read palatial_guide before the first palatial_create_asset call of a session, and read its parameters topic before setting any create field beyond source, name, description, and engine; the create parameters have cross-field rules that reject a request. Generation needs a positive shared organization/workspace net balance and charges only as stages complete; the estimated total is not prepaid. Low recommended balance warnings are advisory. At zero or negative balance, new stages wait while running stages finish. Credit-paused jobs resume on the same asset after posted credit makes the net balance positive. Show billing_guidance and preserve asset IDs; never retry generation, create a replacement, or start checkout automatically. Export itself is free; first export needs a positive balance, and existing exports remain downloadable. A failed asset requires explicit user confirmation and allow_failed_export. An export is not proof of simulator acceptance. This connector contains no proprietary generation prompts.'
  });
  ...
  server.registerTool('palatial_download_asset', {
    description: 'Download an available SimReady export ZIP to output_dir on this computer. Export itself does not consume tokens. A first export needs a positive shared net balance; an existing export remains downloadable at zero or negative balance, including during a later paused run. For PROCESSING_FAILED, first obtain explicit user confirmation, label the result partial or unvalidated, and pass allow_failed_export=true. Saves a SHA-256 receipt, preserves existing files, and reuses a verified local download. Does not extract archives, import into a scene, or certify simulator behavior.',
